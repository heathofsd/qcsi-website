...
**Zero radius, everywhere.** `border-radius: 0` is the system default and there are no exceptions — not on buttons, images, inputs, or video embeds. Paper is cut with a blade, tape is torn by hand, and the floor has no corners.
...
### Artist Entry
The roster is rows, not a grid of portraits — a Courier Prime index number, the artist's name in Archivo, hometown and instrument as mono meta. Opening one navigates to the artist's own page at `/artists/[slug]`; the earlier in-page overlay was retired 2026-08-06 because a page is shareable, indexable for the writer's name, and can carry a press kit, none of which an overlay can.

### Artist Page
The artist's entry in the running order blown up to a full sheet, statically generated from `artists.ts`. Structure: a tape strip carrying a personal fact ("Third year in the round"), the name in display caps, hometown · instrument as run data — then the signature move, **the favorite lyric set in headline size and tape orange**, before the photo and bio. The photograph gets the documentary treatment with a caption rule. On paper: the mono detail list ("The record / On file") closed by one quiet **service row** — "For press & bookers" with a copy-bio button (`CopyButton`) and a photo download. The bio is never displayed twice; an earlier version repeated it under a "Copy-ready" heading and read like an internal memo, which is why the section is a row now, not a section. Then the artist's Invitational record as run rows (a credit that accrues by year), and prev/next rows that walk the 2026 call sheet. Name-only artists get the same head and record with a single-line stand-in body — the URL exists from announcement day. Each page emits Person JSON-LD with `sameAs` socials and a fixed-silhouette OG image (floor, condensed chalk name, one straight tape strip) generated by `opengraph-image.tsx` from instanced static TTFs in `src/assets/` — satori reads neither Google Fonts links nor woff2, and cannot set variable-font axes.
