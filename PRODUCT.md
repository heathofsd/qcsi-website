...
- Logo mark: a crown over "QUEEN CITY SONGWRITERS." Redrawn as vector 2026-08-04, replacing the AI-generated raster original (which had no vector source and irregular letterforms). Lockups live in `public/brand/` — `qcsi-mark.svg` (crown alone), `qcsi-logo-stacked.svg`, `qcsi-logo-horizontal.svg`, plus `qcsi-logo-512.png` for consumers that cannot take SVG and `qcsi-logo-header-2x1.png` (600×300, white ground) for hosts that want a ≥150×75 2:1 header above content. In-app, use the inline components in `src/components/Logo.tsx` so `currentColor` inherits. The wordmark is Archivo outlined at `wght 800 / wdth 88`, matching the site's own type.
